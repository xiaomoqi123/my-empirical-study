...
from src.CommonModules import logger
import src.Androguard.androlyze as An
import src.Modules.BasicBlockAttrBuilder as BasicBlockAttrBuilder
import src.Modules.PScoutMapping as PScoutMapping
...
def GetFromXML( ApkDirectoryPath,ApkFile):
    ...
    try:
        ...
        a = An.APK(ApkFile)
        f = open(os.path.splitext(ApkFile)[0] + ".xml", "wb")
    ...
    try:
        ...
        f.close()
        print(len(RequestedPermissionSet))
        return RequestedPermissionSet, ActivitySet, ServiceSet, ContentProviderSet, BroadcastReceiverSet, HardwareComponentsSet, IntentFilterSet
...
def GetFromInstructions( ApkDirectoryPath,ApkFile, PMap, RequestedPermissionList):
    ...
    try:
        ApkFile = os.path.abspath(ApkFile)
        # apk file obj,dex file obj,dx analysis result
        a, d, dx = An.AnalyzeAPK(ApkFile)
        # print("=====test======")
    except Exception as e:
        # print e
        logger.error(e)
...
def ProcessingDataForGetApkData(ApkDirectoryPath, ApkFile, PMap):
    ...
    try:
        ...
        BroadcastReceiverSet, HardwareComponentsSet,IntentFilterSet = GetFromXML(ApkDirectoryPath, ApkFile)
        ...
        UsedPermissions, RestrictedApiSet, SuspiciousApiSet, URLDomainSet \
            = GetFromInstructions(ApkDirectoryPath,ApkFile, PMap,RequestedPermissionList)
...
if __name__=='__main__':
    # path = r'D:\Program Files\pycharm\drebin-master\drebin'
    # # temp=[]
    # # for APKFileName in os.listdir(path):
    # #     temp.append(os.path.join(path,APKFileName))
    # GetApkData(3,path)
    MalDir = r'F:\pythonSrc\drebin-master\malware'
    # MalDir = r'/home/lxh0319/gxiut/malware'
    GoodDir = r'F:\pythonSrc\drebin-master\benign'
    # GoodDir = r'/home/lxh0319/gxiut/benign'
    GetApkData(3, MalDir,GoodDir)